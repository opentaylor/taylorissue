Configure OpenClaw. Run these 2 commands one at a time.

Command 1:
On macOS/Linux:
  "{openclaw_bin}" onboard --non-interactive --mode local --auth-choice custom-api-key --custom-base-url "{base_url}" --custom-model-id "{model}" --custom-api-key "{api_key}" --custom-provider-id custom --custom-compatibility openai --accept-risk --gateway-port {port} --gateway-bind loopback --skip-skills --skip-channels
On Windows:
  & "{openclaw_bin}" onboard --non-interactive --mode local --auth-choice custom-api-key --custom-base-url "{base_url}" --custom-model-id "{model}" --custom-api-key "{api_key}" --custom-provider-id custom --custom-compatibility openai --accept-risk --gateway-port {port} --gateway-bind loopback --skip-skills --skip-channels
Ignore ALL warnings (e.g. "gateway not reachable", "WSL2"). These are expected.

Command 2:
On macOS/Linux:
  "{openclaw_bin}" config set "models.providers.custom.models[0].contextWindow" 1000000 --strict-json; "{openclaw_bin}" config set "models.providers.custom.models[0].maxTokens" 32768 --strict-json
On Windows:
  & "{openclaw_bin}" config set "models.providers.custom.models[0].contextWindow" 1000000 --strict-json; & "{openclaw_bin}" config set "models.providers.custom.models[0].maxTokens" 32768 --strict-json

Respond with ONLY this JSON:
{"success": true, "config_path": "~/.openclaw/openclaw.json", "details": "configured"}
Only report failure if a command exits with a real error (not warnings).
